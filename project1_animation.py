...
line, = ax.plot(x2,(w[0]-x2*w[1])/w[2])


def init():
    line.set_ydata((w[0]-x2*w[1])/w[2])
    return line,

def update(i):
    global learingtext,listexp,listxy,w,x2
...
def submit():
    ...
    ani=animation.FuncAnimation(fig,update,frames=number,init_func=init,interval=2,blit=True,repeat=False)
    plt.show()
